...
public class AccountController {
    ...
    public DataResponse<String> signIn(@ApiParam(value = "로그인 요청 객체",required = true) @RequestBody @Valid LoginDto dto, Errors errors){
        if (errors.hasErrors())
            throw new CustomDynamicException((new CustomDynamicExceptionState(4000, getErrorString(errors))));
    ...
    public DataResponse<String> changePwd(@ApiParam(value = "비밀번호 변경 객체", required = true) @RequestBody @Valid PasswordUpdateDto dto, Errors errors,
